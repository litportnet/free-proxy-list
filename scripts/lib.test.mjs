...
test('renderReadme replaces marker blocks and leaves the rest of the template untouched', () => {
  ...
  assert.match(readme, /\| List \| Count \| \.txt \| \.json \| \.csv \| Browse online \|/)
  assert.match(readme, /\[View All proxies\]\(https:\/\/litport\.net\/free-proxy\?utm_source=github&utm_medium=repo&utm_campaign=free-proxy-list&utm_content=table-all&checkedWithinMin=30\)/)
  assert.match(readme, /\[View HTTP proxies\]\([^)]*protocol=http&checkedWithinMin=30\)/)
  assert.match(readme, /\[View HTTPS \(CONNECT\) proxies\]\([^)]*list=https&checkedWithinMin=30\)/)
  assert.match(readme, /\[View SOCKS4 proxies\]\([^)]*protocol=socks4&checkedWithinMin=30\)/)
  assert.match(readme, /\[View SOCKS5 proxies\]\([^)]*protocol=socks5&checkedWithinMin=30\)/)
  assert.match(readme, /\[View Fast \(<1 s\) proxies\]\([^)]*list=fast&checkedWithinMin=30\)/)
  assert.match(readme, /\[View Stable \(7-day uptime ≥ 90%\) proxies\]\([^)]*list=stable&checkedWithinMin=30\)/)
  assert.match(readme, /\[View Anonymous\+Elite proxies\]\([^)]*list=anonymous&checkedWithinMin=30\)/)
  assert.match(readme, /\[View United States proxies\]\([^)]*country=us&checkedWithinMin=30\)/)
  assert.match(readme, /HTTP verification endpoint through the proxy/)
