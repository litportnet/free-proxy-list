...
export function buildFileSet(list, { generatedAt, intervalSec, maxAgeMin }) {
  ...
  const stats = {
    ...
    verification: {
      ...
      anonymity_method: 'response headers echoed by the judge are scanned for the checker\'s IP (transparent) and for Via/X-Forwarded-For/Forwarded/Proxy-Connection (anonymous); neither = elite; no headers block = unknown',
      https_method: 'HTTPS request to the judge through the proxy with certificate verification',
...
const downloadRow = (label, file, litportUrl, count) => {
  ...
  return `| ${label} | ${count} | ${cell('txt')} | ${cell('json')} | ${cell('csv')} | [On litport.net](${litportUrl}) |`
}
...
const renderDownloadsBlock = stats => {
  const rows = [
    ['All', 'proxies/all.txt', `https://litport.net/free-proxy?${UTM}&utm_content=table-all`, stats.totals.all],
    ['HTTP', 'proxies/http.txt', `https://litport.net/free-proxy?${UTM}&utm_content=table-http&protocol=http`, stats.totals.http],
    ['HTTPS (CONNECT)', 'proxies/https.txt', `https://litport.net/free-proxy?${UTM}&utm_content=table-https&protocol=http`, stats.totals.https],
    ['SOCKS4', 'proxies/socks4.txt', `https://litport.net/free-proxy?${UTM}&utm_content=table-socks4&protocol=socks4`, stats.totals.socks4],
    ['SOCKS5', 'proxies/socks5.txt', `https://litport.net/free-proxy?${UTM}&utm_content=table-socks5&protocol=socks5`, stats.totals.socks5],
    ['Fast (<1 s)', 'proxies/fast.txt', `https://litport.net/free-proxy?${UTM}&utm_content=table-fast&responseTimeMs=1000`, stats.totals.fast],
    ['Stable (7-day uptime ≥ 90%)', 'proxies/stable.txt', `https://litport.net/free-proxy?${UTM}&utm_content=table-stable&uptimeRating=90`, stats.totals.stable],
    ['Anonymous+Elite', 'proxies/anonymous.txt', `https://litport.net/free-proxy?${UTM}&utm_content=table-anonymous&anonymityLevel=high`, stats.totals.anonymous],
    ['United States', 'proxies/countries/us/all.txt', `https://litport.net/free-proxy?${UTM}&utm_content=table-us&country=us`, stats.countries.us || 0],
  ]
  const header = '| List | Count | .txt | .json | .csv | On litport.net |\n|---|---|---|---|---|---|'
