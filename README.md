...
<!-- VERIFICATION:START -->
- **Judge:** ipkit.io over HTTP through the proxy
...
- **Anonymity method:** response headers echoed by the judge are scanned for the checker's IP (transparent) and for Via/X-Forwarded-For/Forwarded/Proxy-Connection (anonymous); neither = elite; no headers block = unknown
- **HTTPS method:** HTTPS request to the judge through the proxy with certificate verification
